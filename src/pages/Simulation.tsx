  ...
  return (
    <DashboardLayout>
      ...
      <div className="p-4 md:p-6 space-y-4">
            ...
            <CardContent className="p-4 pt-0">
              {!simState.isRunning ? (
                <div className="text-center py-6 text-muted-foreground">
                  ...
                  <p className="text-base">Start simulation to see live statistics</p>
                </div>
              ) : (
        ...
        {simState.isRunning && lots && <CongestionHeatmap lots={lots} />}

        {simState.isRunning && (
          <>
            <div className="space-y-2">
              <h2 className="text-lg font-semibold">Live Parking Lot Status</h2>
              {lotsLoading || !lots ? <Skeleton className="h-64" /> : (
                <ParkingLotsTable 
                  lots={lots}
                  onLotClick={(lotId) => {
                    const lot = lots.find(l => l.id === lotId);
                    if (lot?.activeViolation) navigate(`/violations/${lot.activeViolation.id}`);
                  }}
                />
              )}
            </div>
