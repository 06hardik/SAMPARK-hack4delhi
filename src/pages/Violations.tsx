    ...
    <DashboardLayout>
      ...
      <div className="p-5 md:p-7 space-y-5 md:space-y-6">
        <Tabs defaultValue="all" onValueChange={(v) => setFilter(v as typeof filter)}>
          <TabsList className="w-full sm:w-auto flex-wrap gap-2">
            <TabsTrigger value="all" className="text-sm md:text-base px-3 py-1.5">
              All ({violations?.length || 0})
            </TabsTrigger>
            <TabsTrigger value="active" className="data-[state=active]:text-status-violating text-sm md:text-base px-3 py-1.5">
              Active ({activeCount})
            </TabsTrigger>
            <TabsTrigger value="resolved" className="text-sm md:text-base px-3 py-1.5">
